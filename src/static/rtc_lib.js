class RTC extends EventTarget{
  ...
  addEventListener(type, listener){
    ...
    this.event_target.addEventListener(type, listener);
  }
  ...
  removeEventListener(type, listener){
    ...
    this.event_target.removeEventListener(type, listener);
  }
  ...
  async createAnswer(offer){
    ...
    this.rtc.ondatachannel = function(e){
      ...
      self.channel.tasks = [];
      const default_send = self.channel.send.bind(self.channel);
      self.channel.onbufferedamountlow = function(){
        if(self.channel.tasks.length != 0){
          default_send(self.channel.tasks[0]);
          self.channel.tasks = self.channel.tasks.slice(1, );
        }
      }
      self.channel.send = function(message){

        if(self.channel.bufferedAmount == 0){
          default_send(message);
        }
        else{
          self.channel.tasks.push(message);
        }
      }

      self.channel.onmessage = function (event) {
        self.dispatchEvent(new CustomEvent("message", {
          detail: {data: event.data, id: self.channel.label}
        }));
      };
    ...
    this.rtc.onicegatheringstatechange = function(){
      if(self.rtc.iceGatheringState == "complete"){
        ice_candidates = ice_candidates.filter((item) => {return item.candidate.indexOf("tcp") == -1});
        answer_resolve({
  ...
  createOffer(channel_id){
    ...
    this.channel.tasks = [];
    const default_send = this.channel.send.bind(this.channel);
    this.channel.onbufferedamountlow = function(){
      if(self.channel.tasks.length != 0){
        default_send(self.channel.tasks[0]);
        self.channel.tasks = self.channel.tasks.slice(1, );
      }
    }
    this.channel.send = function(message){
